...
export default function AltButtonStageSetAboutSection() {
    ...
    return (
        <AnimatePresence mode='sync'>
            ...
            className='hidden lg:flex absolute top-[42%] left-[51%]'>
            {storeHandler.stage === 0 ? <Button className={cn(btnStyles)} onClick={() => storeHandler.setStage(1)}>Info</Button> : null}
            {storeHandler.stage === 1 ? <Button className={cn(btnStyles)} onClick={() => storeHandler.setStage(2)}>Info</Button> : null}
            {storeHandler.stage === 2 ? <Button className={cn(btnStyles)} onClick={() => storeHandler.setStage(0)}>Info</Button> : null}
            </motion.span>
            <motion.span 
